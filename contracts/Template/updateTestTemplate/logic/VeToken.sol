...
contract VeTokenV2 is IVeToken , VeTokenDataV2 {
    ...
    function claim() external  returns (uint256) {
        ...
        if (amount != 0) { 
            ...
            if(totalReward >0 && totalReward >= amount){
        ...
        returns (Claimable memory)
    {
        ...
        for (uint256 i = 0; i < 52; i++) {
            ...
            if (weekCursor >= userPoint.ts && userEpoch <= maxUserEpoch) {
                ...
                if (_tbalanceOf > 0 && _veSupply > 0) {
                    ...
                    toDistribute += ((_tbalanceOf * MULTIPLIER /_veSupply) * (_temp.linearWeeklyRelease * uint(dt)))/MULTIPLIER;
                }
                n++;
                weekCursor += WEEK;
            }
        ...
        return Claimable(toDistribute, userEpoch, maxUserEpoch, weekCursor,_temp.linearWeeklyRelease,_temp.balanceOfDatas,_temp.veSupplyOfDatas,_temp.dtOfDatas);
    }
